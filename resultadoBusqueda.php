      ...
      <table style="width: 100%;">
        ...
        <tbody style="background-color: #ff9800; ">
          <tr> <?php if(isset($_POST) && $contador != 11 && $encontrado != false){  foreach ($encontrado as $valor){ foreach($valor as $key => $value){ ?>
            <th scope="col"> 
              <?php if($key == "id_operacion"){ ?> <form action="modificar.php" method="POST"><button type="submit" name="<?php echo $key; ?>" value="<?php echo $value; ?>">Modificar</button></form><form action="eliminarTicket.php" method="POST"><button type="submit" name="<?php echo $key; ?>" value="<?php echo $value; ?>">Elminar</button></form> <?php } else{ echo $value; }?> </th>
