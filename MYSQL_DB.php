...
class MYSQL_DB{
  ...
  public function agregarRemito($data)
  {
    ...
    if (!$stmt->execute()) {
      Echo "Hubo un error al cargar ticket, intente de nuevo más tarde";
    }
  }
  ...
  public function eliminatTicket($id_operacion)
  {
    $stmt = $this->con->prepare("DELETE FROM clientes WHERE id_operacion = :id_operacion");
    $stmt->bindValue(":id_operacion", $id_operacion, PDO::PARAM_STR);
    $stmt->execute();
  }
